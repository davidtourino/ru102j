...
public class SiteGeoDaoRedisImpl implements SiteGeoDao {
    ...
    public Set<Site> findAll() {
        try (Jedis jedis = jedisPool.getResource()) {
            ...
            List<Response<Map<String, String>>> resultList = new ArrayList<>();
            for (String key : keys) {
                resultList.add(pipeline.hgetAll(key));
            }
            pipeline.sync();
            for (Response<Map<String, String>> mapResponse : resultList) {
                Map<String, String> site = mapResponse.get();
                if (!site.isEmpty()) {
...
//     }
    ...
    private Set<Site> findSitesByGeoWithCapacity(GeoQuery query) {
        ...
        GeoUnit radiusUnit = query.getRadiusUnit();
         ...
         try (Jedis jedis = jedisPool.getResource()) {
             // START Challenge #5
             List<GeoRadiusResponse> radiusResponses = jedis.georadius(RedisSchema.getSiteGeoKey(), query.getCoordinate().getLng(), query.getCoordinate().getLat(), query.getRadius(), query.getRadiusUnit());
             // END Challenge #5
             ...
             for (Site site : sites) {
                 Response<Double> zscore = pipeline.zscore(RedisSchema.getCapacityRankingKey(), String.valueOf(site.getId()));
